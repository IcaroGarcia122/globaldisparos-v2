...
async function getResend(): Promise<any> {
  ...
  try {
    ...
    logger.warn('[Email] Pacote "resend" não instalado — rode: npm install resend');
    return null;
  }
}
...
async function send(to: string, subject: string, html: string): Promise<boolean> {
  ...
  if (!client) {
    logger.warn(`[Email] Não enviado para ${to} — RESEND_API_KEY não configurada`);
    return false;
  }
  try {
    const { error } = await client.emails.send({ from: FROM, to, subject, html });
    if (error) { logger.error(`[Email] Erro: ${JSON.stringify(error)}`); return false; }
    logger.info(`[Email] ✅ Enviado para ${to}: ${subject}`);
    return true;
  } catch (err: any) {
    logger.error(`[Email] Falha: ${err.message}`);
    return false;
  }
}

export async function sendPasswordReset(to: string, resetLink: string, name?: string): Promise<boolean> {
...
export async function sendWelcome(to: string, name: string, plan: string): Promise<boolean> {
...
export async function sendInvite(to: string | null, inviteLink: string, plan: string, note?: string): Promise<boolean> {
  if (!to) return false;
  const labels: Record<string,string> = { basic:'Basic', pro:'Pro', enterprise:'Enterprise' };
