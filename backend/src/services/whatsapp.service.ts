...
class WhatsAppService {
  ...
  constructor() {
    ...
    this.client.interceptors.response.use(
      ...
      (err) => {
        ...
        logger.warn(`[Evolution] ${err.config?.method?.toUpperCase()} ${err.config?.url} → ${err.response?.status}: ${msg}`);
  ...
  async logoutInstance(name: string): Promise<void> {
    try { await this.client.delete(`/instance/logout/${name}`); } catch { /* ignora */ }
  }

  async deleteInstance(name: string): Promise<void> {
    try { await this.client.delete(`/instance/delete/${name}`); } catch { /* ignora */ }
  }

  async registerWebhook(name: string): Promise<void> {
    const webhookUrl = process.env.WEBHOOK_URL || 'http://host.docker.internal:3001/api/webhook/evolution';
    const events = ['QRCODE_UPDATED', 'CONNECTION_UPDATE', 'GROUPS_UPSERT', 'GROUP_UPDATE', 'GROUP_PARTICIPANTS_UPDATE', 'MESSAGES_UPSERT'];
    ...
    try {
      ...
      logger.info(`[Evolution] Webhook registrado para ${name}`);
    } catch (err: any) {
      ...
      try {
        ...
        logger.info(`[Evolution] Webhook registrado para ${name} (formato v1)`);
      } catch (err2: any) {
        logger.warn(`[Evolution] Webhook registration failed for ${name}: ${err2.message}`);
      }
    }
  }
  ...
  async fetchGroups(instanceName: string): Promise<any[]> {
    ...
    try {
      ...
      const groups = raw.filter((g: any) => (g.id || g.jid || '').includes('@g.us'))
        .map((g: any) => ({
          id: g.id || g.jid,
          subject: g.subject || g.name || g.pushName || `Grupo ${(g.id||g.jid||'').slice(-8)}`,
          size: g.size || g.participants?.length || 0,
        }));
      logger.info(`[Evolution] ${groups.length} grupos via fetchAllGroups`);
      return groups;
    } catch (err: any) {
      logger.warn(`[Evolution] fetchAllGroups falhou (${err.message}) — tentando findChats`);
    }
    ...
    try {
      ...
      logger.info(`[Evolution] ${groups.length} grupos via findChats (fallback)`);
      return groups;
    } catch (err: any) {
      logger.warn(`[Evolution] findChats falhou: ${err.message}`);
      return [];
    }
  }
   ...
   */
  async enrichGroupNamesViaMessages(instanceName: string, instanceId: number, prismaClient: any): Promise<void> {
    ...
    logger.info(`[Evolution] Buscando nomes via mensagens para ${semNome.length} grupos...`);
      ...
      await Promise.all(batch.map(async (g: any) => {
        try {
          const res = await this.client.post(
            ...
            { where: { key: { remoteJid: g.groupId } }, limit: 1 },
            { timeout: 5000 }
          );
    ...
    logger.info(`[Evolution] Nomes via mensagens: ${updated}/${semNome.length} atualizados`);
  }
   ...
   */
  async enrichGroupNames(instanceName: string, instanceId: number, prismaClient: any): Promise<void> {
    ...
    if (!groups.length) {
      logger.info(`[Evolution] Todos os grupos já têm nome real — enriquecimento desnecessário`);
      return;
    }

    logger.info(`[Evolution] Enriquecendo ${groups.length} grupos sem nome para ${instanceName}`);
    let updated = 0;
    const BATCH = 5;
    const DELAY = 500;
      ...
      await Promise.all(batch.map(async (g: any) => {
        try {
          ...
          const r = await this.client.get(
            `/group/findGroupInfos/${instanceName}/${encodeURIComponent(g.groupId)}`,
            { timeout: 8000 }
          );
          const data = r.data;
          if (data?.subject) {
    ...
    logger.info(`[Evolution] Enriquecimento: ${updated}/${groups.length} grupos atualizados`);
  }
  ...
  async getGroupsWhereAdmin(instanceName: string, ownerPhoneFromDb?: string): Promise<Array<{ groupId: string; name: string; participantsCount: number }>> {
    ...
    try {
      ...
      if (!ownerPhone) {
        logger.warn(`[AdminGroups] ownerPhone vazio para ${instanceName}`);
        return [];
      }
  ...
  async addParticipants(instanceName: string, groupJid: string, participants: string[]): Promise<{ success: string[]; failed: string[] }> {
    ...
    logger.info(`[AddParticipants] Tentando direto: POST ${directUrl}`);
    try {
      const res  = await fetch(directUrl, { method: 'POST', headers, body: directBody });
      const text = await res.text();
      logger.info(`[AddParticipants] Direto HTTP ${res.status}: ${text.slice(0, 400)}`);

      if (res.ok) {
        let data: any = {};
        try { data = JSON.parse(text); } catch {}
        const results: any[] = Array.isArray(data) ? data : (data?.updateParticipants || []);
        ...
        if (results.length > 0) {
          ...
          logger.info(`[AddParticipants] Direto: ${success.length} ok, ${failed.length} falhas`);
          return { success, failed };
        }
        // Resposta vazia = sucesso
        return { success: participants, failed: [] };
      }
      logger.warn(`[AddParticipants] Direto retornou ${res.status} — tentando link de convite`);
    } catch (e: any) {
      logger.warn(`[AddParticipants] Direto falhou: ${e.message} — tentando link de convite`);
    }

    // ── Método 2: Buscar link de convite e enviar para cada número ────────────
    // Funciona mesmo quando updateParticipant não existe na versão
    logger.info(`[AddParticipants] Usando link de convite para ${groupJid}`);
    try {
      ...
      logger.info(`[AddParticipants] InviteCode HTTP ${inviteRes.status}: ${inviteText.slice(0, 200)}`);
      ...
      const inviteCode: string = inviteData?.inviteCode || inviteData?.code || inviteData?.invite_code || '';
      if (!inviteCode) throw new Error('Link de convite não retornado');

      const inviteLink = `https://chat.whatsapp.com/${inviteCode}`;
      logger.info(`[AddParticipants] Link: ${inviteLink}`);
      ...
      for (const phone of participants) {
        ...
        try {
          ...
          if (msgRes.ok) {
            success.push(number);
            logger.info(`[AddParticipants] ✉️  Convite enviado para ${number}`);
          } else {
            failed.push(number);
          }
        } catch {
      ...
      logger.info(`[AddParticipants] Convite: ${success.length} enviados, ${failed.length} falhas`);
      return { success, failed };
    } catch (e: any) {
      logger.warn(`[AddParticipants] Link de convite falhou: ${e.message}`);
