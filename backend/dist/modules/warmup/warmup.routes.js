...
async function restoreActiveWarmups() {
    ...
    `.catch(() => []);
        for (const row of rows) {
            ...
            logger_1.default.info(`[Warmup] ♻️  Retomado para ${row.instance_name} (dia ${day}, total ${state.totalSent})`);
        }
        if (rows.length > 0)
            logger_1.default.info(`[Warmup] Auto-retomada: ${rows.length} warmup(s) restaurado(s)`);
    }
    catch (err) {
        logger_1.default.warn(`[Warmup] Auto-retomada falhou: ${err.message}`);
    }
}
// Executar após 3s para dar tempo do Prisma conectar
setTimeout(restoreActiveWarmups, 3000);
// ─── Loop de envio ────────────────────────────────────────────────────────────
async function sendNextBatch(key) {
    ...
    for (let i = 0; i < batchSize; i++) {
        ...
        try {
            await whatsapp_service_1.default.sendText(state.instanceName, state.phone, msg);
            state.sessionSent++;
            state.totalSent++;
            ...
            (0, socket_server_1.emitToUser)(state.userId, 'warmup:progress', {
            ...
            logger_1.default.info(`[Warmup] ✅ ${state.instanceName} → ${state.phone}: "${msg}"`);
            database_1.default.$executeRaw `UPDATE warmup_states SET total_sent=${state.totalSent}, session_sent=${state.sessionSent}, updated_at=NOW() WHERE instance_id=${state.instanceId}`.catch(() => { });
        }
        catch (err) {
            addLog(state, `Erro: ${err.message}`, phase.id, 'error');
            logger_1.default.warn(`[Warmup] ❌ Erro: ${err.message}`);
            (0, socket_server_1.emitToUser)(state.userId, 'warmup:error', { instanceId: state.instanceId, error: err.message });
...
function addLog(state, msg, phaseId, type) {
    const time = new Date().toLocaleTimeString('pt-BR', { hour: '2-digit', minute: '2-digit', second: '2-digit' });
    state.logs.push({ time, msg, phaseId, type });
    if (state.logs.length > 100)
        state.logs.shift(); // mantém últimas 100
}
// ─────────────────────────────────────────────────────────────────────────────
// ROTAS
// ─────────────────────────────────────────────────────────────────────────────
...
router.post('/start', async (req, res) => {
    ...
    const instance = await database_1.default.whatsAppInstance.findUnique({ where: { id: instId } });
    if (!instance)
        return res.status(404).json({ error: 'Instância não encontrada' });
    if (instance.status !== 'connected')
        return res.status(409).json({ error: 'Instância não conectada' });
    if (!instance.phoneNumber)
        return res.status(400).json({ error: 'Número não disponível' });
    const key = `${instId}`;
    ...
    const savedRows = await database_1.default.$queryRaw `SELECT * FROM warmup_states WHERE instance_id = ${instId} LIMIT 1`.catch(() => []);
    ...
    if (saved && saved.started_at) {
        ...
        logger_1.default.info(`[Warmup] Retomando do banco: dia ${resumeDay}, totalSent ${resumeTotalSent}`);
    }
    else {
        // Início limpo
    ...
    state.timer = setTimeout(() => sendNextBatch(key), firstDelay);
    logger_1.default.info(`[Warmup] Iniciado para ${instance.name || 'instance_' + instId} → ${instance.phoneNumber} (dia ${startDay})`);
    return res.json({
...
router.post('/stop', async (req, res) => {
    ...
    const state = activeWarmups.get(key);
    if (!state)
        return res.status(404).json({ error: 'Nenhum warmup ativo' });
    state.running = false;
    if (state.timer)
  ...
  `.catch(() => { });
    logger_1.default.info(`[Warmup] Parado para instance_${instanceId} (total: ${state.totalSent})`);
...
router.get('/status', async (req, res) => {
    ...
    if (instanceId) {
        ...
        if (!state) {
            ...
            const rows = await database_1.default.$queryRaw `SELECT * FROM warmup_states WHERE instance_id = ${parseInt(key)} LIMIT 1`.catch(() => []);
...
router.post('/reset-started-at', async (req, res) => {
  ...
  `.catch(() => { });
    ...
    logger_1.default.info(`[Warmup] startedAt corrigido para instance ${instId}: dia ${newDay}`);
    return res.json({ ok: true, newDay, newStartedAt, message: `Aquecimento agora está no dia ${newDay}` });
});
exports.default = router;
//# sourceMappingURL=warmup.routes.js.map